...
- [EncounterSetType Policy and Direction](docs/01-Adding_Images/encounter_set_types.md) - Encounter-set schema, custody persistence, metadata contracts, verification serial numbering and required routing fields, and phased delivery model
...
- [Project Data Sync API](docs/API/project_sync/README.md) - Email- and admin-approved desktop mirror of a project's encounters, images, gradings, annotations (COCO) and workbook, with the `scripts/project_sync_client.py` client
...
- [Grading Workbench Save-Latency Optimization Plan](docs/15-DEVELOPMENT/grading_workbench_save_latency_plan.md) - Data-backed plan to reduce draft and Save & Next query amplification while preserving atomicity, idempotency, authorization, and GET-only history restoration
- [Madhunetra WAI DR-DME Encounter Adapter Plan](docs/15-DEVELOPMENT/madhunetra_wai_dr_dme_adapter_plan.md) - Encounter-scoped combined DR/DME inference, project workflow configuration, Remidio/OCR triggers, image-level result persistence, and manual batching plan
...
- [CSV-Driven EncounterSet Import Plan](docs/15-DEVELOPMENT/csv_driven_encounter_set_import_plan.md) - Admin-reviewed EncounterSetType inference, frozen mapper revisions, manifest staging, resumable OD/OS image reconciliation, safe deletion, and verification-gated finalization
...
- [Discrepancy Review API](docs/API/discrepancy-review/README.md) - Project-aware task filters, saved review evidence, permission-scoped study CSV queues, idempotent submissions, and MV-independent Save & Next ordering
...
- [Project Annotation Policy and GPU Grading Workbench](docs/16-NewFeature/imageMarking/06-project-annotation-policy.md) - Project-resolved tools, task-level grade/class/annotation lifecycle, normalized annotations, and full-resolution segmentation masks
