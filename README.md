...
- [Grader Responsiveness and Performance Plan](docs/15-DEVELOPMENT/grader_responsiveness_performance_plan.md) - Phased plan to keep grader-facing latency flat as data grows: server process model, per-request session overhead, image payload sizing under an authenticate-every-image constraint, bounded task acquisition, and non-blocking materialized view refresh
