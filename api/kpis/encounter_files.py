...
@with_session()
def year_month_wise_uploads(db):
    ...
    try:
        ...
        ).join(EncounterFile).join(LabUnit).join(Hospital).group_by(
...
def dr_reports_count(db):
    ...
    try:
        ...
        encounters_with_dr = query.join(DiabeticRetinopathyReport).filter(
        ...
        ).join(DiabeticRetinopathyReport).group_by(
            extract('month', PatientEncounters.capture_date_dt)
        ).order_by(extract('month', PatientEncounters.capture_date_dt)).all()
        
        # By hospital
        by_hospital = query.with_entities(
            Hospital.id.label('hospital_id'),
            Hospital.name.label('hospital_name'),
            func.count(DiabeticRetinopathyReport.id).label('count')
        ).join(DiabeticRetinopathyReport).join(LabUnit).join(Hospital).group_by(
        ...
        by_lab_unit = query.with_entities(
            ...
            LabUnit.name.label('lab_unit_name'),
            func.count(DiabeticRetinopathyReport.id).label('count')
        ).join(DiabeticRetinopathyReport).join(LabUnit).group_by(
...
def glaucoma_reports_count(db):
    ...
    try:
        ...
        encounters_with_glaucoma = query.join(GlaucomaReport).filter(
        ...
        by_hospital = query.with_entities(
            ...
            Hospital.name.label('hospital_name'),
            func.count(GlaucomaReport.id).label('count')
        ).join(GlaucomaReport).join(LabUnit).join(Hospital).group_by(
            Hospital.id, Hospital.name
        ).all()
        ...
        by_lab_unit = query.with_entities(
            ...
            LabUnit.name.label('lab_unit_name'),
            func.count(GlaucomaReport.id).label('count')
        ).join(GlaucomaReport).join(LabUnit).group_by(
            LabUnit.id, LabUnit.name
        ).all()
...
@roles_required("admin", "data_manager")
@with_session()
def dr_results_distribution(db):
    ...
    try:
        ...
        # Base query with permissions
        query = db.query(DiabeticRetinopathyReport).join(PatientEncounters)
        query = apply_user_permissions(query, user_lab_unit_ids, is_admin)
        query = apply_location_filters(query, params)
        query = apply_date_filters(query, params)
        ...
        monthly_trends = query.with_entities(
            func.date_trunc('month', PatientEncounters.capture_date_dt).label('month'),
            ...
            func.date_trunc('month', PatientEncounters.capture_date_dt)
        ).order_by('month').all()
...
@roles_required("admin", "data_manager")
@with_session()
def glaucoma_results_distribution(db):
    ...
    try:
        ...
        # Base query with permissions
        query = db.query(GlaucomaReport).join(PatientEncounters)
        query = apply_user_permissions(query, user_lab_unit_ids, is_admin)
        query = apply_location_filters(query, params)
        query = apply_date_filters(query, params)
...
def vcdr_distribution(db):
    ...
    try:
        ...
        # Base query with permissions
        query = db.query(GlaucomaResultsCleaned).join(PatientEncounters)
        query = apply_user_permissions(query, user_lab_unit_ids, is_admin)
        query = apply_location_filters(query, params)
        query = apply_date_filters(query, params)
        ...
        right_eye_stats = query.with_entities(
            func.avg(GlaucomaResultsCleaned.vcdr_right_num).label('mean'),
            func.percentile_cont(0.5).within_group(GlaucomaResultsCleaned.vcdr_right_num).label('median'),
            func.stddev(GlaucomaResultsCleaned.vcdr_right_num).label('std_dev')
        ).filter(GlaucomaResultsCleaned.vcdr_right_num.isnot(None)).first()
        
        # Get distribution ranges for right eye
        right_eye_ranges = query.with_entities(
            func.count(func.case(
                (GlaucomaResultsCleaned.vcdr_right_num < 0.5, 1),
                else_=0
            )).label('normal_0_5'),
            func.count(func.case(
                (and_(GlaucomaResultsCleaned.vcdr_right_num >= 0.5, GlaucomaResultsCleaned.vcdr_right_num < 0.7), 1),
                else_=0
            )).label('borderline_0_5_0_7'),
            func.count(func.case(
                (and_(GlaucomaResultsCleaned.vcdr_right_num >= 0.7, GlaucomaResultsCleaned.vcdr_right_num < 0.8), 1),
                else_=0
            )).label('abnormal_0_7_0_8'),
            func.count(func.case(
                (GlaucomaResultsCleaned.vcdr_right_num >= 0.8, 1),
                else_=0
            )).label('severely_abnormal_gt_0_8')
        ).filter(GlaucomaResultsCleaned.vcdr_right_num.isnot(None)).first()
        
        # Get statistics for left eye
        left_eye_stats = query.with_entities(
            func.avg(GlaucomaResultsCleaned.vcdr_left_num).label('mean'),
            func.percentile_cont(0.5).within_group(GlaucomaResultsCleaned.vcdr_left_num).label('median'),
            func.stddev(GlaucomaResultsCleaned.vcdr_left_num).label('std_dev')
        ).filter(GlaucomaResultsCleaned.vcdr_left_num.isnot(None)).first()
        
        # Get distribution ranges for left eye
        left_eye_ranges = query.with_entities(
            func.count(func.case(
                (GlaucomaResultsCleaned.vcdr_left_num < 0.5, 1),
                else_=0
            )).label('normal_0_5'),
            func.count(func.case(
                (and_(GlaucomaResultsCleaned.vcdr_left_num >= 0.5, GlaucomaResultsCleaned.vcdr_left_num < 0.7), 1),
                else_=0
            )).label('borderline_0_5_0_7'),
            func.count(func.case(
                (and_(GlaucomaResultsCleaned.vcdr_left_num >= 0.7, GlaucomaResultsCleaned.vcdr_left_num < 0.8), 1),
                else_=0
            )).label('abnormal_0_7_0_8'),
            func.count(func.case(
                (GlaucomaResultsCleaned.vcdr_left_num >= 0.8, 1),
                else_=0
            )).label('severely_abnormal_gt_0_8')
        ).filter(GlaucomaResultsCleaned.vcdr_left_num.isnot(None)).first()
...
def lab_unit_performance(db):
    ...
    try:
        ...
        ).outerjoin(DiabeticRetinopathyReport).outerjoin(GlaucomaReport).join(LabUnit).join(Hospital).group_by(
