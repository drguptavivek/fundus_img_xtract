   ...
   # Session cookie hygiene
    ...
    def homepage():
        ...
        with Session() as db:
            ...
            disease_distribution = db.execute(
                select(PatientEncounters.name, func.count(PatientEncounters.id))
                .group_by(PatientEncounters.name)
                .order_by(func.count(PatientEncounters.id).desc())
                .limit(10)
            ).all()
