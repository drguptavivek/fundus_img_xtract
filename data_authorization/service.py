"""Persisted project-role grants and their project/Lab Unit containment rules."""
...
PROJECT_GRANT_MANAGER_ROLES = PI_DELEGATORS | PROJECT_ADMIN_DELEGATORS
...
def upsert_project_role_grant(
    ...
    *,
    ...
    db.refresh(row, attribute_names=["project", "user", "role", "lab_unit"])
...
def replace_project_role_grants(
    ...
    *,
    ...
    actor_assignable_roles = _actor_assignable_role_names(
        db,
        actor=actor,
        data=scope_input,
    )
    ...
    if unsupported:
        raise ProjectGrantPermissionDenied(
            "One or more roles cannot be assigned within the requested scope."
        )
    if not actor.has_role("admin") and user_id == actor.id:
        raise ProjectGrantPermissionDenied("Non-admin users cannot grant roles to themselves.")
    if not actor_assignable_roles:
    ...
    original_scope = (original_scope_type, original_lab_unit_id)
    requested_scope = (scope_type, lab_unit_id)
    if original_scope_type and original_scope != requested_scope:
        ...
        actor_assignable_original = _actor_assignable_role_names(
            db,
            actor=actor,
            data=original_input,
        )
        old_rows = db.execute(
            ...
            .where(
                ...
                Role.name.in_(actor_assignable_original),
                _exact_scope_clause(
                    scope_type=original_scope_type,
                    lab_unit_id=original_lab_unit_id,
                ),
...
def grant_to_dto(row: ProjectRoleGrant) -> ProjectRoleGrantDTO:
    ...
    return ProjectRoleGrantDTO(
        ...
        role_name=row.role.name,
        scope_type=row.scope_type,
        hospital_id=(row.lab_unit.hospital_id if row.lab_unit else None),
        hospital_name=(
            row.lab_unit.hospital.name if row.lab_unit and row.lab_unit.hospital else None
        ),
        lab_unit_id=row.lab_unit_id,
...
def _validate_scope_target(db: Session, data: ProjectRoleGrantInput) -> None:
    if data.scope_type not in PROJECT_SCOPE_TYPES:
        raise ProjectGrantValidationError("scope_type must be project or lab_unit.")
    ...
    if data.scope_type == PROJECT_SCOPE:
        if data.lab_unit_id is not None:
            raise ProjectGrantValidationError("Project scope cannot include a lab unit.")
    else:
        if data.lab_unit_id is None:
...
def _exact_scope_clause(*, scope_type: str, lab_unit_id: int | None):
...
def _log_grant_edit(*, actor: User, row: ProjectRoleGrant, change: str) -> None:
    _LOGGER.info(
        ...
        "role=%s scope_type=%s lab_unit_id=%s active=%s",
...
def _require_manage_scope(db: Session, *, actor: User, data: ProjectRoleGrantInput) -> None:
    manager_grants = _delegator_grants(db, actor=actor, project_id=data.project_id)
    if can_delegate_project_role(
        target_role=data.role_name,
        actor_user_id=actor.id,
        target_user_id=data.user_id,
        actor_is_admin=actor.has_role("admin"),
        requested_scope_type=data.scope_type,
        requested_lab_unit_id=data.lab_unit_id,
        delegator_grants=manager_grants,
    ):
        return
    raise ProjectGrantPermissionDenied("You cannot manage the requested project scope.")
...
def _manageable_grant_clause(db: Session, *, actor: User, project_id: int):
    ...
    for manager in manager_grants:
        if manager.scope_type == PROJECT_SCOPE:
            conditions.append(
                or_(
                    ProjectRoleGrant.scope_type == PROJECT_SCOPE,
                    and_(
                        ...
                        ProjectRoleGrant.lab_unit_id.in_(
                            configured_project_lab_unit_ids(db, project_id=project_id)
                            or {-1}
                        ),
                    ),
                )
            )
        elif manager.scope_type == LAB_UNIT_SCOPE and manager.lab_unit_id is not None:
            conditions.append(
                and_(
                    ProjectRoleGrant.scope_type == LAB_UNIT_SCOPE,
                    ProjectRoleGrant.lab_unit_id == manager.lab_unit_id,
                )
            )
    return or_(*conditions) if conditions else None


def _actor_assignable_role_names(
    db: Session,
    *,
    actor: User,
    data: ProjectRoleGrantInput,
) -> frozenset[str]:
    """Return roles the actor may assign at this exact contained scope."""
    return delegable_project_roles(
        actor_user_id=actor.id,
        target_user_id=data.user_id,
        actor_is_admin=actor.has_role("admin"),
        requested_scope_type=data.scope_type,
        requested_lab_unit_id=data.lab_unit_id,
        delegator_grants=_delegator_grants(
            db, actor=actor, project_id=data.project_id
        ),
    )


def _delegator_grants(
    db: Session, *, actor: User, project_id: int
) -> tuple[DelegatorGrant, ...]:
    rows = db.execute(
        select(Role.name, ProjectRoleGrant.scope_type, ProjectRoleGrant.lab_unit_id)
        .join(ProjectRoleGrant, ProjectRoleGrant.role_id == Role.id)
        .where(
            ProjectRoleGrant.project_id == project_id,
            ProjectRoleGrant.user_id == actor.id,
    ...
    return tuple(
        DelegatorGrant(role_name=name, scope_type=scope_type, lab_unit_id=lab_id)
        for name, scope_type, lab_id in rows
    )
