...
5.  **Deduplication & Hashing**:
    ...
    -   Checks the `DirectImageUpload` table for matches solely via the **truncated hash** to detect duplicates (file size is passed during the request but deduplication relies on the content hash).
6.  **Image Processing & Storage**:
    -   **EXIF Stripping**: Discards all technical metadata by reconstructing pixels using `strip_exif_data`. (Note: This is mandatory for direct clinical uploads; see [Pre-graded Workflow](03-Pre_Graded_Workflow.md) for exceptions).
...
1.  **Quota Management**: Priority-based quota system (User-specific > System-wide setting > Unlimited).
2.  **Synchronous Engine**: Unlike Zip uploads, primary validation and processing happen within the HTTP request for immediate feedback.
3.  **Security Suite**:
    -   **Filename Sanitization**: Multi-layer protection (Custom validation + Werkzeug `secure_filename`).
    -   **MIME Sniffing**: Rejects files with spoofed extensions.
    -   **EXIF Stripping**: Permanent removal of camera/patient technical metadata.
    -   **Strong Hashing**: Uses SHA-256 for integrity and collision resistance.
4.  **Metadata Extraction**: Synchronous extraction of technical diagnostic metrics using `utils/image_metadata.py`.
5.  **Anonymization Pipeline**:
    -   **PII Detection**: Asynchronous OCR-based text scanning.
    -   **Editing System**: Allows for manual pixel modifications (masking) while maintaining full audit logs and metadata consistency.
