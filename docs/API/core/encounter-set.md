...
| `/api/v1/encounter-set/<uuid>/details` | `GET` | Session + login + `admin`, `local_admin`, `optometrist` | Path `uuid` | `{ "uuid": str, "patient_id": str, "patient_name": str, "capture_date": date \| datetime, "images": [{"uuid": str, "spatial_position": int, "url": str, "thumbnail_url": str \| null}] }` | `404` if the encounter is not found or not in scope. `403` on role failure. `500` on unexpected server error. |
...
| `/api/v1/encounter-set/upload` | `POST` | Bearer JWT via `@token_auth_required` + rate limit (`60 per minute`) | Multipart form-data | `{ "message": str, "encounter_id": int, "encounter_uuid": str, "image_uuid": str, "spatial_position": int }` | `400`, `401`, `403`, `404`, `413`, or `500` depending on validation and storage failure. |
