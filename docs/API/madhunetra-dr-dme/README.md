...
The existing EncounterSet inference browser at `/uploads/encountersets/wadhwani_inference` has a workflow selector. `?workflow=dr_dme` shows only scoped projects with manual DR-DME enabled and defaults to fully eligible encounters. Its search supports eligibility (`eligible`, `any`, `not_verified`, or `binocular_one_eye` for non-monocular encounters with macula images from only one eye), capture-date range, camera, DR OCR report availability, exclusion or inclusion of prior DR/DME runs, and page sizes of 25, 50, 75, or 100 EncounterSets. Applied filters and pagination are persisted in the page URL and restored on reload or browser back/forward navigation; the Glaucoma inference browser follows the same URL-history behavior. Candidate cards show age, sex, canonical monocular status, report summary, eligibility blockers, and prior run/report state; macula images are grouped into side-by-side OD and OS columns with equal 150-pixel square thumbnails. The result header can select every visible, enabled encounter, while each card's accessible header checkbox controls that encounter without redundant label text. A header **View** button opens the matching project/month/date/encounter browser detail in a new tab. Selecting an EncounterSet still queues all its eligible macula images as one combined screening request; filters never split the encounter submission. The Glaucoma workflow exposes the equivalent select-all-visible and header **View** navigation for its image candidates.
...
Job and polling routes resolve the workflow from the persisted job type when `workflow=dr_dme` is absent. This keeps direct/bookmarked job URLs valid and prevents the shared status page from treating an encounter-scoped DR/DME job as a task-scoped Glaucoma job.
...
The DR-DME batch status summary reports positive encounters and positive images from persisted mapped results. An image is positive when either its DR or DME result indicates disease; each encounter is counted once regardless of how many of its images are positive. Explicit no-disease and ungradable results are not counted as positive.
