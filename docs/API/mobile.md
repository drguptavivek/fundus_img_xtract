...
| `/auth/sessions` | `GET` | Bearer access token | None | `{"sessions":[{"id","device_id","device_name","created_at","updated_at","last_used_at","refresh_token_expires_at","allowed_lab_unit_ids","allowed_disease_ids","is_revoked","current"}]}` | `401`, `403` |
| `/auth/sessions/<session_id>` | `DELETE` | Bearer access token | Path `session_id` | `204 No Content` | `401` |
