...
/**
 * Initializes hospital and lab unit filters
 * Now hospital-aware: Uses /api/user/hospital-context and updated APIs
 */
async function initializeFilters() {
    try {
        // Get user's hospital context first
        const contextResponse = await fetch('/api/user/hospital-context');
        const context = await contextResponse.json();

        console.log('User hospital context:', context);

        // Store context globally for use in other functions
        window.userHospitalContext = context;

        // Get eligible lab units and hospitals (now hospital-filtered)
        const response = await fetch('/api/eligibleLabUnitCurrentUser');
        const data = await response.json();
        ...
        if (hospitalSelect) {
            const hospitals = data.eligible_hospitals || [];
            ...
            // Hide hospital filter for non-admin users
            if (!context.is_master_admin) {
                const hospitalFilterGroup = hospitalSelect.closest('.form-group');
                if (hospitalFilterGroup) {
                    hospitalFilterGroup.style.display = 'none';
                    console.log('Hospital filter hidden for non-admin user');
                }
            }

            // Pre-select user's hospital if they have one
            if (context.hospital_id && !context.is_master_admin) {
                hospitalSelect.value = context.hospital_id;
            }
        }

        // Populate lab unit dropdown
        const labUnitSelect = document.getElementById('labUnitFilter');
        if (labUnitSelect) {
            const labUnits = data.eligible_lab_units || [];

            // Clear existing options except "All"
            labUnitSelect.innerHTML = '<option value="">All Lab Units</option>';

            labUnits.forEach(labUnit => {
                const option = document.createElement('option');
                option.value = labUnit.id;
                option.textContent = labUnit.name;
                // Optionally show hospital name for master admin
                if (context.is_master_admin && labUnit.hospital_name) {
