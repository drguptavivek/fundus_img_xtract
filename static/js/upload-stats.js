(() => {
  const matrixRoot = document.getElementById('upload-stats-matrix');
  ...
  if (!matrixRoot || !zipWeekMyRoot || !zipWeekAllRoot || !zipTodayMyRoot || !zipTodayAllRoot || !directTodayMyRoot || !directTodayAllRoot || !directWeekMyRoot || !directWeekAllRoot) return;
  ...
  const renderMatrix = (matrix) => {
    const cards = [
      ...
      { label: 'Total ZIPs Cumulative', value: matrix.total.cumulative.zip, style: 'kpi--info' },
      { label: 'Total Direct Today', value: matrix.total.today.direct, style: 'kpi--success' },
      { label: 'Total Direct Cumulative', value: matrix.total.cumulative.direct, style: 'kpi--warning' },
      { label: 'Total Pregraded Today', value: matrix.total.today.pregraded, style: 'kpi--teal' },
      { label: 'Total Pregraded Cumulative', value: matrix.total.cumulative.pregraded, style: 'kpi--secondary' },
    ];

    const grid = document.createElement('div');
    grid.className = 'row g-3 row-cols-1 row-cols-sm-2 row-cols-lg-3';
  ...
  const init = async () => {
    try {
      ...
      renderError(matrixRoot, err.message || 'Failed to load upload stats');
    ...
    try {
      ...
      renderZipDaily(weeklyData.data.zip_daily.my, zipWeekMyRoot);
      renderZipDaily(weeklyData.data.zip_daily.all, zipWeekAllRoot);
