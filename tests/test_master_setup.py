...
def create_master_data():
    ...
    try:
        ...
        # Create disease gradings for Glaucoma
        glaucoma = next((d for d in diseases if d.name == 'Glaucoma'), None)
        if glaucoma:
            glaucoma_gradings_data = [
                {'impression': 'Normal', 'display_order': 1, 'is_active': True},
                {'impression': 'Glaucoma Suspect', 'display_order': 2, 'is_active': True},
                {'impression': 'Glaucoma', 'display_order': 3, 'is_active': True},
                {'impression': 'Other Retinal', 'display_order': 4, 'is_active': True},
                {'impression': 'Not gradable', 'display_order': 5, 'is_active': True}
            ]
            
            for grading_data in glaucoma_gradings_data:
                grading = db.execute(
        ...
        # Create disease gradings for Diabetic Retinopathy
        dr = next((d for d in diseases if d.name == 'Diabetic Retinopathy'), None)
        if dr:
            dr_gradings_data = [
                {'impression': 'No DR', 'display_order': 1, 'is_active': True},
                {'impression': 'Mild NPDR', 'display_order': 2, 'is_active': True},
                {'impression': 'Moderate NPDR', 'display_order': 3, 'is_active': True},
                {'impression': 'Severe NPDR', 'display_order': 4, 'is_active': True},
                {'impression': 'PDR', 'display_order': 5, 'is_active': True},
                {'impression': 'Not gradable', 'display_order': 6, 'is_active': True}
            ]
            
            for grading_data in dr_gradings_data:
                grading = db.execute(
                    select(DiseaseGrading).where(
