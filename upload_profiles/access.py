...
def uploader_qualification_clause(
    *, user_id: int, role_names: frozenset[str] = frozenset({"fileUploader"})
):
...
def is_uploader_qualified(
    db, *, user_id: int, role_names: frozenset[str] = frozenset({"fileUploader"})
) -> bool:
    """Return whether the current user may combine an upload assignment."""
    return bool(
        db.execute(
            select(
                uploader_qualification_clause(user_id=user_id, role_names=role_names)
            )
        ).scalar()
    )
...
def _assignment_query(
    *,
    ...
    role_names: frozenset[str] = frozenset({"fileUploader", "admin"}),
):
...
def has_upload_assignment(
    ...
    *,
    ...
    qualification_roles: frozenset[str] | None = None,
) -> bool:
    ...
    role_names = qualification_roles or (
        frozenset({"pregarded_uploader", "admin"})
        if kinds == {"pregraded"}
        else frozenset({"fileUploader", "admin"})
    )
...
def upload_assignment_lab_clause(
    *,
    user_id: int,
    lab_unit_column,
    qualification_roles: frozenset[str] = frozenset({"fileUploader", "admin"}),
):
    ...
    return exists(
        _assignment_query(user_id=user_id, role_names=qualification_roles).where(
