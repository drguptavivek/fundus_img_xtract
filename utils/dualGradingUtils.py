...
from models import GradingTask, User, UserDiseaseUnitRole, EncounterFile, DirectImageUpload, Disease, LabUnit
...
def get_all_pending_resident_for_disease(db, user_id: int, disease_id: int) -> Dict[str, int]:
    """
...
def get_all_pending_faculty_for_disease(db, user_id: int, disease_id: int) -> Dict[str, int]:
    """
    ...
    Returns:
        A dictionary with 'total' (total count of pending tasks)
    """
    # Check if user has faculty role (ophthalmologist)
    user = db.query(User).options(selectinload(User.roles)).filter(User.id == user_id).first()
    if not user:
        return {'total': 0}
...
def get_all_pending_arbitration_for_disease(db, user_id: int, disease_id: int) -> Dict[str, int]:
    """
    ...
    Returns:
        A dictionary with 'total' (total count of pending tasks)
    """
    # Check if user has arbitration role (ophthalmologist)
    user = db.query(User).options(selectinload(User.roles)).filter(User.id == user_id).first()
    if not user:
        return {'total': 0}
...
def get_all_pending_resident_for_labUnit_disease(db, user_id: int, lab_unit_id: int, disease_id: int) -> Dict[str, Optional[int]]:
    """
...
def get_all_pending_faculty_for_labUnit_disease(db, user_id: int, lab_unit_id: int, disease_id: int) -> Dict[str, Optional[int]]:
    """
...
def get_all_pending_arbitration_for_labUnit_disease(db, user_id: int, lab_unit_id: int, disease_id: int) -> Dict[str, Optional[int]]:
    """
