    ...
    *,
    ...
    if session is None:
        ...
        with get_db_session() as new_session:
            task = get_next_intra_rater_task(user_id, disease_id, db=new_session)
            if task is not None:
                # Detach so callers can safely access attributes after the session closes.
                new_session.expunge(task)
            return task
