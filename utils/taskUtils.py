...
def get_task_summary(
    ...
    try:
        ...
        query = session.query(Task).join(LabUnit).join(Disease)
...
def get_tasks_by_status(
    ...
    try:
        # Join with appropriate image association
        query = session.query(Task).join(LabUnit).join(Disease).outerjoin(Image, Task.encounter_file_id == Image.id).outerjoin(DirectImageUpload, Task.direct_image_upload_id == DirectImageUpload.id)
...
def get_task_stats(lab_unit_ids: Optional[List[int]] = None) -> Dict[str, int]:
...
def get_tasks_for_user(
    ...
    try:
        ...
        user_disease_unit_roles = session.query(UserDiseaseUnitRole).filter(
